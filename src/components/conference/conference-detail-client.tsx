    ...
    const fetchConference = async () => {
      ...
      try {
        ...
        if (result.success && result.conference) {
          ...
          setError(result.message || 'Conference could not be loaded.');
          toast({
            title: "Error",
        ...
        setError("Could not fetch conference details.");
        toast({
          title: "Error",
          description: message,
          variant: "destructive",
  ...
  if (error || !conference) {
    return (
      <div className="container py-12 md:py-24 text-center">
        ...
        <p className="text-muted-foreground mt-2">{error || "The conference you are looking for does not exist or could not be loaded."}</p>
      </div>
    );
  }
